...
async def test_save_request_to_mongo(database_service: DatabaseServiceSQL) -> None:
    ...
    correlation_id = "".join(random.choices(string.hexdigits, k=7))  # noqa: S311 doesn't need to be secure
    ...
    sim: Simulation = await database_service.insert_simulation(sim_request, correlation_id)
    ...
    assert sim2.sim_request.omex_archive == get_slurm_sim_experiment_dir(get_correlation_id(correlation_id))
