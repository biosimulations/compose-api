...
async def nats_container_uri() -> AsyncGenerator[str]:
    with NatsContainer() as nats_container:
        yield nats_container.nats_uri()


@pytest_asyncio.fixture(scope="function")
async def nats_subscriber_client(nats_container_uri: str) -> AsyncGenerator[NATSClient]:
...
@pytest_asyncio.fixture(scope="function")
async def nats_producer_client(nats_container_uri: str) -> AsyncGenerator[NATSClient]:
    client = await nats.connect(nats_container_uri, verbose=True)
    yield client
    await client.close()
