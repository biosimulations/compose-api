...
# version is an optional argument, defaults to the version defined in compose_api/version.py'
#
# version.py is of form:
# __version__ = "0.1.0"
declared_version=$(grep -oE '__version__ = \"[^\"]+\"' "${ROOT_DIR}/compose_api/version.py" | awk -F'"' '{print $2}')
...
for service in api; do
  ...
  image_name="ghcr.io/biosimulations/compose-${service}:${tag}"
