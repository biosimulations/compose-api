...
RUN micromamba run -p /micromamba_env/runtime_env pip install /code/$${#BASE_PATH}
...
RUN micromamba update -c conda-forge -p /micromamba_env/runtime_env $${#DEPENDENCIES}
    ...
    python_string = "python3" if has_no_conda else "micromamba run -p /micromamba_env/runtime_env python"
    entrypoint_str = 'ENTRYPOINT ["python3", "/runtime/main.py"]' if has_no_conda else 'ENTRYPOINT ["micromamba", "run", "-p", "/micromamba_env/runtime_env", "python", "/runtime/main.py"]'
